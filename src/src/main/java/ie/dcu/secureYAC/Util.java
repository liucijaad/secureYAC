...
public final class Util {
    ...
    public static byte[] HKDF(byte[] keyInputMaterial, byte[] salt, String info)
        throws NoSuchAlgorithmException {
        HKDFParameters param = new HKDFParameters(keyInputMaterial, salt,
            info.getBytes());
    ...
    public static KeyBundle loadFromFile(String fileName) throws Exception {
        ...
        byte[] firstKey = Util.bigIntToByteArray(
            new BigInteger(reader.nextLine(), 16), 256);
        byte[] secondKey = Util.bigIntToByteArray(
            new BigInteger(reader.nextLine(), 16), 256);
        if(fileName.contains(".id")) {
            ...
            byte[] preKeySignature = Util.bigIntToByteArray(
                new BigInteger(reader.nextLine(), 16), 512);
            byte[] otpk = Util.bigIntToByteArray(
                new BigInteger(reader.nextLine(), 16), 256);
            
            keyBundle = new PreKeyBundle(username, firstKey, secondKey,
                preKeySignature, otpk);
        }
        reader.close();
        return keyBundle;
    }

    public static int writeToFile(String fileName, String fileType,
        String data, int fileCount) throws IOException {
